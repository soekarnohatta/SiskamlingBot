...
func (*Module) ping(ctx *telegram.TgContext) error {
	...
	text := fmt.Sprintf("<b>Recorded Timespan Is:</b> <code>%v</code> ", timeEnd.String())
...
func (m *Module) helpCallback(ctx *telegram.TgContext) error {
	...
	match := pattern.FindStringSubmatch(ctx.Callback.Data)[1]
	...
	case "main":
		...
		_, _, err := ctx.Callback.Message.EditText(
			ctx.Bot,
			text,
	...
	default:
		...
		text, keyb := telegram.CreateMenuf("./data/menu/help/"+match+".json", 2, dataMap)
		if text == "" {
